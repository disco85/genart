...
def get_segments(ps, ifseg=None):
  p2 = 1
  segi = 0
  res = []
  if ifseg is None:
...
def draw_segs(dr, segs, fill='green', width=1):
  for seg in segs:
    dr.line(seg, fill=fill, width=width)

def draw_hexagon(dr, hg, ifseg=None, overlay=False):
  #segs = itertools.pairwise(hg['points'])
  ps = [cartesian_to_canvas(p) for p in hg['points']]
  ps.append(ps[0])
  width = ROD + 4; color = RODBORDERCOLOR
  draw_segs(dr, get_segments(ps, ifseg), fill=color, width=width) #dr.line(ps, fill=color, width=width)
  if not overlay:
    for p in ps:
      draw_circle(dr, p, width/2 - 1, fill=RODBORDERCOLOR, width=1)
  width = ROD; color = RODCOLOR
  draw_segs(dr, get_segments(ps, ifseg), fill=color, width=width) #dr.line(ps, fill=color, width=width)
  if not overlay:
    for p in ps:
      draw_circle(dr, p, width/2 - 1, fill=RODCOLOR, width=1)

def draw_hexagons(dr, hgs):
  even = lambda i: i%2
  for hgi, hg in enumerate(hgs):
    draw_hexagon(dr, hg, ifseg=lambda i: even(hgi) and even(i))
    # draw_hexagon(dr, hg, ifseg=lambda i: not even(hgi) and not even(i), overlay=True)
    # draw_hexagon(dr, hg, ifseg=lambda i: even(hgi) and not even(i), overlay=True)
    #draw_hexagon(dr, hg, ifseg=lambda i: even(hgi) and not even(i), overlay=True)
  # FIXME still wrong order: мб просто нумеровать отрезки глобально?
  # for hgi, hg in enumerate(hgs):
  #   draw_hexagon(dr, hg, ifseg=lambda i: not even(hgi) and not even(i%2),
  #                overlay=True)
  # for hgi, hg in enumerate(hgs):
  #   draw_hexagon(dr, hg, ifseg=lambda i: even(hgi) and even(i%2),
  #                overlay=True)
  # for hgi, hg in enumerate(hgs):
  #   draw_hexagon(dr, hg, ifseg=lambda i: not even(hgi) and even(i%2),
  #                overlay=True)
